...
describe('initial', () => {
  it('returns initial state when session storage is empty', () => {
    item = null
    let state = localStorageYobta({ channel: 'yobta' }).initial('yobta')
...
describe('next', () => {
  it('sets ls item', () => {
    localStorageYobta({ channel: 'yobta' }).next('yobta')
...
describe('observe', () => {
  it('subscribes, receives update, and unsubscribes', () => {
    ...
    let channel = localStorageYobta({ channel: 'yobta' })
...
describe('encoder', () => {
  ...
  it('decodes initial', () => {
    localStorageYobta({ channel: 'yobta', encoder }).initial('yobta')
  ...
  it('encodes next', () => {
    localStorageYobta({ channel: 'yobta', encoder }).next('yobta')
  ...
  it('encodes observer message', () => {
    localStorageYobta({ channel: 'yobta', encoder }).observe(() => {})
    windowMock.addEventListener.mock.calls[0][1]({
      key: 'yobta',
      newValue: JSON.stringify('yobta'),
    })
